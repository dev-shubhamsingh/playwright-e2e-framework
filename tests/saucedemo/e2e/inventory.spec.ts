...
test.describe('Inventory', () => {
  ...
  test.describe('Product display', () => {
    ...
    test('all expected product names are present', async ({ authenticatedPage }) => {
    ...
    test('cart badge is hidden when cart is empty', async ({ authenticatedPage }) => {
  ...
  test.describe('Sorting', () => {
    ...
    test('sorts products by price low to high', async ({ authenticatedPage }) => {
    ...
    test('sorts products by price high to low', async ({ authenticatedPage }) => {
  ...
  test.describe('Add and remove from cart', () => {
    ...
    test('adding multiple products updates the badge count', async ({ authenticatedPage }) => {
      await authenticatedPage.addToCartByName(PRODUCTS.backpack.name);
    ...
    test('badge disappears after removing the last item', async ({ authenticatedPage }) => {
      await authenticatedPage.addToCartByName(PRODUCTS.backpack.name);
      expect(await authenticatedPage.getCartCount()).toBe(1);
  ...
  test.describe('Navigation', () => {
    ...
    test('clicking a product opens its detail page', async ({ authenticatedPage, productDetailPage }) => {
