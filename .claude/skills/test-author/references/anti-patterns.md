...
| Asserting a value you read manually when a locator matcher exists       | `expect(await p.getTitle()).toBe('X')` reads once; it cannot retry, so it fails on a slow paint                                         | `await expect(p.pageTitle).toHaveText('X')` — expose the locator                                                                          |
| **Using `.resolves` and believing it retries**                          | It does **not**. `expect(promise).resolves` awaits one promise, exactly like `expect(await …)`. Swapping one for the other buys nothing | A locator matcher, or `await expect.poll(() => p.getX()).toBe(y)` for a derived value                                                     |
