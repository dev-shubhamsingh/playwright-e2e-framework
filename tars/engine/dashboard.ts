...
import { fmtMs, escapeHtml } from '../lib/format.ts';
...
export function bars(rows: { name: string; count: number }[]): string {
...
function main(): void {
  ...
  // Only rendered when a relative report path is supplied (the Pages publish
  // step sets this). Omitted locally, where the link would be dead.
  const reportPath = process.env.TARS_REPORT_PATH;
  const reportLink = reportPath
    ? ` · <a href="${escapeHtml(reportPath)}" style="color:var(--accent)">full Playwright report →</a>`
    : '';
  ...
  <div class="wrap">
    ...
    <div class="panel">
      ...
      ${d.slowest
        .map(
          (s) =>
            `<div class="row"><span>${escapeHtml(s.title)} <span class="muted">· ${escapeHtml(s.project)}</span></span><span class="muted">${fmtMs(s.durationMs)}</span></div>`,
        )
        .join('')}
    </div>
    ...
    ${
      d.flaky.length
        ? `<div class="panel"><h2>⚠️ Flake watch — quarantine candidates</h2>${d.flaky
            .map(
              (f) =>
                `<div class="row flake"><span>${escapeHtml(f.title)}</span><span class="muted">${escapeHtml(f.project)}</span></div>`,
            )
            .join('')}</div>`
        : ''
    }

    <footer>Generated by TARS Mission Control · ${d.passed} passed · ${d.failed} failed · ${d.skipped} skipped${reportLink}</footer>
  </div>
</body>
</html>`;
...
// Only run the CLI when executed directly, so `bars()` above can be imported by
